...
I protipi di funzione sono parte delle dichiarazioni:
...
Un prototipo associato a una dichiarazione in avanti deve corrispondere al prototipo
associato alla dichiarazione della funzione; in caso contrario, Perl genera un warning.
Curiosamente, potete omettere il prototipo nella dichiarazione in avanti e
includerlo nella dichiarazione vera e propria--ma non ci sono ragioni per farlo.

X<builtin; C<prototype>>
...
di tipo. Non servono nè a documentare il numero o il tipo degli argomenti delle funzioni,
nè a mappare gli argomenti su parametri per nome.

Le coercizioni di tipo causate dai prototipi hanno comportamenti sottili, come quello di forzare il contesto scalare sugli
argomenti:
...
prototipo invece dell'operatore virgola. È un altro dei problemi della sintassi dei protipi.
...
con C<sort>N<Questo esempio è stato suggerito Ben Tilly.>:
