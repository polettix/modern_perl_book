...
Perl 5 è già di per se stesso un linguaggio pragmatico, ma la sua comunità
ne ha notevolmente esteso le funzionalità rendendo poi disponibile al resto del mondo il proprio lavoro.
Se avete un problema, è probabile che qualcuno abbia già scritto--e condiviso--del codice Perl che lo risolve.

X<CPAN>

La programmazione in Modern Perl fa un uso estensivo di CPAN (U<http://www.cpan.org/>).
...
qualunque cosa: dall'accesso ai database ai tool di profiling, dai protocolli per 
...
Senza CPAN, il Modern Perl semplicemente non sarebbe l'incredibile linguaggio che è.
...
collaboratori, 101656 moduli e 23808 distribuzioni (con incrementti dell'11.5%, 19.4% e 14.3% rispetto agli stessi dati nell'edizione precedente di
questo libro).
...
Il sistema CPAN è semplicemente un servizio di mirroring. GLi autori fanno l'upload delle distribuzioni
e CPAN le invia ai mirror, da cui gli utenti e i programmi client possono scaricarli e
configurarli, farne il build, testarli e installarli. Il successo di questo sistema è docuto sia alla sua
semplicità che al contributo delle migliaia di volontari che lo hanno adottato per distribuire il proprio codice. 
In particolare, si sono sempre più affermati nella comunità certi attributi e caratteristiche standard che una distribuzione CPAN deve avere. Questi includono:
...
U<http://rt.cpan.org/> per riportare bug e lavorare con gli autori per la loro soluzione. 
I siti CPAN forniscono inoltre link alle versioni precedenti delle distribuzioni, valutazioni dei moduli scritte dai suoi utenti,
documentazione e altro ancora. Tutto questo è disponibile su U<http://search.cpan.org/>.

X<CPAN; C<CPAN.pm>>
X<CPAN; C<CPANPLUS>>

Le installazioni di Modern Perl contengono due client per connettersi a CPAN e cercare, scaricare,
...
(al momento presente, l'ultima release stabile è la 1.9800), l'installazione di moduli è ragionevolemnte facile.
...
contiene un'eccellente sezione sul troubleshooting.

=begin sidebar

Anche se il client CPAN è ormai integrato nella distribuzione di Perl 5, potreste
dovere installare alcuni tool standard di sviluppo, come l'utility C<make>
...
installate XCode. Se siete utenti Unix o Unix-like dovreste già avere questi tool installati sul vostro sistema 
(per gli utenti Debian e Ubuntu: installate C<build-essential>).

=end sidebar

=head2 Tool per la Gestione di CPAN

Se il vostro sistema operativo possiede una sua installazione di Perl 5, questa potrebbe essere
obsoleta o dipendere da specifiche versioni di alcune distribuzioni CPAN. Gli sviluppatori Perl esperti spesso frappongono delle barriere virtuali
 tra il Perl di sistema e le loro installazioni Perl di sviluppo. Ci sono diversi progetti che vi aiuteranno a raggiungere questo scopo.
...
La sua installazione si fa semplicemente scrivendo:
...
distribuzioni nella vostra directory utente, anzichè nell'intero sistema.
...
L'installazione è un pó più complicata di quella delle due distribuzioni descritte sopra, ma C<App::local::lib::helper> vi semplificherà il processo.  Vedete
U<http://search.cpan.org/perldoc?local::lib> e
U<http://search.cpan.org/perldoc?App::local::lib::helper> per maggiori dettagli.

Tutti e tre i progetti assumono che lavoriate in ambiente Unix-like (come
