...
contenuto di C<$_> in assenza di una variabile esplicita. Potete usare C<$_> come un'altra variabile, ma spesso è inutile.

X<builtins; C<chomp>>

Per esempio, la funzione nativa C<chomp> rimuove una sequenza finale di newline dalla
stringa dataN<See C<perldoc -f chomp> e C<$/> per maggiori dettagli.>:
...
=begin programlisting
    ...
    print;  # stampa $_ sul filehandle attualmente selezionato
    say;    # stampa $_ sul filehandle attualmente selezionato
...
Se C<calculate_value()> o qualche altra funzione modifica C<$_>, tale modifica persiste
...
Perl 5.12 ha introdotto l'operatore triplo-punto (C<...>). È una istruzione completa
di per sè, che serve come segnaposto per del codice che volete scrivere successivamente.
In fase di parsing, Perl lo tratta come un'istruzione, ma solleva un'eccezione
sul vostro tentativo di eseguire del codice non implementato se tentate di eseguirlo. Vedete
...
Perl fornisce anche due variabili array implicite. Anzitutto, Perl passa gli argumenti alle
funzioni (L<functions>) in un array named C<@_>. Le operazioni di manipolazione di array
(L<arrays>) all'interno delle funzioni per default lavorano su questo array, per cui i due seguenti
pezzetti di codice sono equivalenti:
...
Perl tratterà ogni elemento di C<@ARGV> come se fosse il I<nom> di un file da aprire in lettura. (Se C<@ARGV> è vuoto, Perl leggerà da standard input.) Questo
...
C<readline $fh> ha lo stesso effetto di C<< <$fh> >>. A partire da 5.10, la chiamata
C<readline> senza argomenti è equivalente a C<< <> >>, quindi ora potete usare C<readline>
dappertutto. Per ragioni storiche, l'uso di C<< <> >> è ancora più diffuso, ma siete invitati a
prendere in considerazione l'uso C<readline> per la sua maggiore leggibilità. Probabilmente preferite C<glob
