...
X<classi>
X<istruzioni predefinite; C<package>>
...
Ora tutte le istanze di C<Gatto> possono svegliarvi al mattino perchè non hanno
ancora mangiato:
...
I metodi che accedono ai dati dell'invocante sono I<metodi d'istanza>, perchè dipendono dalla
presenza dell'invocante appropriato per funzionare correttamente. I metodi (come
C<miagola()>) che non accedono a i dati d'istanza sono I<metodi di classe>. Potete
...
=begin programlisting
    ...
    for my $nome (qw( Tuxie Petunia Daisy ))
    {
        ...
        say "Creato un gatto per ", $cat->nome();
    }
...
L'uso di C<ro> oppure di C<rw> è determinata da scelte di progetto, di convenienza e di purezza di stile. Moose
...
classe con un oggetto di un'altra classe purchè forniscano la stessa interfaccia
esterna.
...
qualunque oggetto che può fare C<qua()> è sufficientemente simile a un'anatra perchè possiate trattarlo
...
ovvie, che funzionano tutte con questo metodo purchè si conformino al comportamento
atteso.

Immaginate di dover passare in rassegna tutti gli animali di uno zoo senza poter
usare una funzione poliforma. I benefici della genericità dovrebbero essere evidenti. Inoltre, gli
...
di per se stessa implicazioni sul comportamento di un oggetto. Un oggetto C<Cane> potrebbe avere un
...
sia un C<EssereVivente> rimane valido. C<Gatto> può decidere se implementare l'età da sè oppure
...
di comportamenti--una cosa molto utile per testare le capacità anzichè le
implementazioni.
...
nomi e i comportamenti, ma l'ereditarietà è più familiare agli sviluppi con esperienza in
...
Come per gli attributi, le sottoclassi possono sovrascrivere dei metodi. Imamaginate una luce che non potete
spegnere:
...
assemblarlo alla meno peggio a partire da diverse distribuzioni CPAN. Moode rappresenta un
...
Perl e ActivePerl lo includono. Anche se Moose è un modulo CPAN anzichè una libreria
