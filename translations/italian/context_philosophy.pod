...
Se assegnate il valore di ritorno della funzione a un singolo elemento (L<scalars>) valuta
la funzione in I<contesto scalare>:
...
Se assegnate il resultato della chiamata alla funzione ad un array (L<arrays>) o a una
...
Tutto questo non è forse ovvio, ma ora che lo sapete, la differenza di contesto di numero tra queste due istruzioni dovrebbe esservi evidente:
...
Valutare una funzione o una espressione--eccetto negli assegnamenti--in contesto lista può
...
e decidere di conseguenz quanto lavoro deve fare per restituire il risultato.
...
tipo di contesto che desiderate. Supponiamo che vogliate confrontare il cuntenuto di due stringhe.
