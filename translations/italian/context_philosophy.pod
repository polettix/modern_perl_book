...
=begin programlisting
    ...
    (
        ...
        oprazione_costosa       => cerca_faccende(), # OPS!
    );

=end programlisting

Quest'ultimo esempio sorprende spesso i programmatori principianti che si aspettano un contesto scalare
per quella chiamata di funzione. Invece, inizializzare un hash (L<hashes>) con una lista di valori
impone un contesto lista alla chiamata a C<cerca_faccende>.
...
e decidere di conseguenza quanto lavoro fare per restituire il risultato.
...
Perl fa del suo meglio per convertire i valori al tipo appropriato (L<coercion>),
...
I contesti di tipo sono meno difficili da identificare rispetto ai contesti di quantità. Una volta che
sapete della loro esistenza e conoscete i contesti forniti dai vari operatori (L<operator_types>), è raro che possiate sbagliarvi.
