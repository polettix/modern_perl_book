...
Una I<variable> in Perl è un'area per la memorizzazione di un valore (L<values>). Mentre
...
(L<lexical_scope>). Gli stessi I<file sorgente> forniscono il proprio scope lessicale, mentre
...
=begin programlisting
    ...
    # $discount ancora visibile
    say "Il nostro sconto è attualmente $discount!";
...
=begin programlisting
    ...
    package Store::Music
    {
        # $discount non disponibile
    }

    package Store::BoardGame;

    # $discount sempre non disponibile
...
C<@values>. Accedete al suo primo elemento--un singolo valore--con
C<$values[0]>. E accedete a una lista di valori dentro l'array con C<@values[ @indices
]>.
...
programmatori, a tener traccia di una C<$mela>, dei C<@barili> o dei C<%ristoranti_economici>.
Le variabili create I<senza> un nome dal vostro codice sorgente sono dette variabili I<anonime>.
L'inico modo per accedere a una variabile anonima è per riferimento
(L<references>).
