...
X<variabili globali predefinite>
X<variabili; globali predefinite>

Perl 5 fornisce svariate I<variabili globali predefinite> il cui scope è realmente globale e non
...
=head2 Gestire le Variabili Globali Predefinite

X<variabili globali predefinite; gestione>
...
è possibile evitare di usare molte di queste variabili globali. Quando però non potete evitarle, usate C<local> nello scope
...
Non tutti i casi di utilizzo delle variabili globali predefiniti sono così facili da trattare, ma spesso questo approccio
funziona.
...
Talvolta potete aver bisogno di I<leggere> il valore di una variabile globale predefinita, sperando che nel frattempo
...
Il modulo C<English> distribuito con Perl fornisce dei nomi verbosi per le variabili globali predefinite al posto di quelli composti di punteggiatura. Potete importarli in un namespace con:
...
Tre variabili globali predefinite relative alle regex (C<$&>, C<$`> e C<$'>) impongono una penalità
...
=head2 Variabili Predefinite Utili

X<variabili globali predefinite; utili>

Molti programmi in Perl 5 moderno possono cavarsela usando solo un paio delle variabili predefinite
globali. Probabilmente, incontrerete solo alcune di queste variabili nei programmi reali.

X<C<$/>>
X<variabili globali; C<$/>>
X<C<$INPUT_RECORD_SEPARATOR>>
X<variabili globali; C<$INPUT_RECORD_SEPARATOR>>
X<C<readline>>
...
X<variabili globali; C<$INPUT_LINE_NUMBER>>
...
X<variabili globali; C<$OUTPUT_AUTOFLUSH>>
...
X<variabili globali; C<$!>>
X<C<$ERRNO>>
X<variabili globali; C<$ERRNO>>

=item * C<$!> (C<$ERRNO>) è una dualvar (L<dualvars>) contenente il risultato
...
X<variabili globali; C<$">>
X<C<$LIST_SEPARATOR>>
X<variabili globali; C<$LIST_SEPARATOR>>
...
=head2 Alternative alle Variabili Predefinite

X<variabili globali predefinite; alternative>
X<C<Try::Tiny>>
