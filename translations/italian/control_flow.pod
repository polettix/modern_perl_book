...
=begin programlisting
    ...
    say "Tu non sei Bob!" unless $name eq 'Bob';
...
=begin programlisting
    ...
    my $bbq;
    if (defined $bbq and $bbq eq 'salsiccia') { ... }
...
=begin programlisting
    ...
    is( $nums[0], 1, '1 * 1 vale 1' );
    is( $nums[1], 4, '2 * 2 vale 4' );

    ...

    is( $nums[9], 100, '10 * 10 vale 100' );
...
confusione. Considerate una funzione C<topic_mangler()> il cui scopo è di
modificare C<$_>. Se il codice che itera su una lista chiama C<topic_mangler()> senza
...
non ha di per sè alcun side effect. Ovvero, se C<@values> contiene uno o più elementi,
...
C<@values> modificandolo ad ogni iterazione del ciclo:
...
È anche possibile modificare C<@values> all'interno della condizione del C<while>,
...
Potete nidificare i cicli dentro altri cicli:
...
Un errore comune quando si nidificano un ciclo C<foreach> e un C<while> è che è facile
...
=begin programlisting
    ...
    {
        # NON USARE; probabilmente contiene bug
        while (<$fh>)
        {
            say $prefix, $_;
        }
...
Usare le istruzioni di controllo di ciclo nei cicli nidificati può creare confusione. Se non potete
evitare di nidificare i cicli--trasformando i cicli interni in funzioni--usate una
...
=begin programlisting
    ...
    while (<$fh>)
    {
        ...
        for my $prefix (@prefixes)
        {
            ...
            say "$prefix: $_";
            # qui c'è un next PREFIX implicito
        }
    }
...
=begin programlisting
    ...
    {
        is( $username, $_, 'assegnamento automatico a variabile topic' );
    }
...
C<given> è particolarmente utile quando è usato in combinazione con C<when>. 
...
=begin programlisting
    ...
    my @options  = ( \&sasso, \&carta, \&forbici );
    my $confused = "Non capisco la tua mossa.";
...
Il costrutto C<when> è ancora più potente di quanto mostrato fin qui; può fare un match (L<smart_match>)
con molti altri tipi di espressioni inclusi scalari, aggregati,
riferimenti, espressioni di confronto arbitrarie e anche riferimenti a codice.
...
un'altra funzione--il volaore di ritorno della funzione chiamante è il valore
...
In questo codice, ritornare da C<greet_person()> direttamente al chiamante di
C<log_and_greet_person()> è più efficiente che ritornare a
C<log_and_greet_person()> e poi ritornare immediatamente I<da>
C<log_and_greet_person()>. Ritornare direttamente da C<greet_person()> al
chiamante di C<log_and_greet_person()> è una I<ottimizzazione di chiamata di coda>.
...
Purtroppo, il Perl 5 non riesce sempre a identificare i casi dove potrebbe applicare questa
ottimizzazione automaticamente. Se questo succede col vostro codice, fate a mano una ottimizzazione
di chiamata di coda.
...
Questo esempio mostra due aspetti importanti. Prima di tutto, C<goto &function_name> o
C<goto &$function_reference> richiede di usare il sigillo di funzione (C<&>) in
