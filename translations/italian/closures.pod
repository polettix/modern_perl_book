...
Ricordate che non potete I<nidificare> le funzioni con nome. Le funzioni con nome hanno uno scope
globale al package. La condivisione di ogni variabile lessicale condivisa tra funzioni nidificate viene
...
richiesto. Ritarda le computazioni fino a quando non sono assolutamente necessarie.

Forse tutto questo può sembrare eccessivo per calcolare una semplice serie numerica, ma è
la funzione C<gen_fib()> in sè ad essere notevole. Inizializza un array come cache,
esegue del codice dedicato per popolare elementi arbitrari della cache e
restituisce il valore calcolato o estratto dalla cache. Se astraete dalle operazioni specifiche per la serie
di Fibonacci, questo codice può fornire un meccanismo pigro di cache del codice arbitrario.

L'estrazione di una funzione generica C<genera_chiusura_cache()> produce:
...
In un certo senso, le funzioni native C<map>, C<grep> e C<sort> sono esse stesse delle
funzioni di ordine superiore. Confrontatele con C<genera_chiusura_cache()>.
...
I<Higher Order Perl> di Mark Jason Dominus è il riferimento bibliografico canonico per imparare a scrivere
funzioni e chiusure in Perl di ottimo livello. Potete leggerlo online all'URL
