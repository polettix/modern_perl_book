...
=begin programlisting
    ...
    my $reminder = 'LB<\'>apice va preceduto '
...
=begin programlisting
    ...
    my $exception = 'Questa stringa termina con un '
...
=begin programlisting
    ...
    my $literal = "due
    linee";
    is $escaped, $literal, 'equivalenza \n e newline';
...
=begin programlisting
    ...
    my $factoid = "B<$name> vive a B<$address>!";

    # equivalente a
    my $factoid = $name . ' vive a ' . $address . '!';
...
di caretteri bilanciati--come aperta e chiusa graffa--il corrispondente carattere di chiusura
...
=begin programlisting
    ...
    my $quote     = B<qq{>"Ahia", disse.  "Fa I<male>!"B<}>;
    my $reminder  =  B<q^>L'apice singolo non va quotato!B<^>;
    my $complaint =  B<q{>È un po' troppo presto per svegliarsi.B<}>;
...
La sintassi di C<< <<'END_BLURB' >> ha tre parti. Le doppie parentesi angolari
...
C<END_BLURB> è un identificatore arbitrario che il parser Perl 5 usa come
delimitatore finale.
...
Perchè I<ottetti> e non I<byte>? L'assunzione che un carattere possa stare dentro un byte può essere causa
...
o dalla rete) come sequanze di ottetti, Perl non può sapere se i dati che leggete
costituiscono un file immagine o un documento di testo o qualunque altra cosa. Per default, Perl
tratta tutti i dati in input come sequanze di ottetti. Tocca a voi assegnare un significato specifico
a tali dati.
...
warning (C<Wide character in %s>), perchè il file contiene ottetti, e non
caratteri Unicode.
...
è quello di usare la pragma C<utf8> (L<pragmas>), che indica al parser Perl di
...
dei codici Unicode. Usate la pragma C<charnames> per attivarli e la sequanza di escape
...
dei caratteri Unicode, Perl codifica la stringa usando UTF-8, perchè Latin-1
...
Se C<$name> contiene un nome Inglese come I<Alice> non si riscontra nessun problema,
...
C<$name> contiene quattro caratteri Unicode.

=item *

C<$name> contiene quattro ottetti Latin-1 che rappresentano quattro caratteri Unicode.

=item *

C<$name> contiene cinque ottetti UTF-8 che rappresentano quattro caratteri Unicode.

=back

Ci sono diversi scenari anche per la costante stringa:
...
perchè è attiva la pragma C<utf8> o C<encoding>.
...
Se sia C<$hello> che C<$name> sono stringhe Unicode, la loro concatenazione produce
un'altra stringa Unicode.

Se entrambe le stringhe sono sequanze di ottetti, Perl le concatenerà in una nuova sequenza
...
I<JosE<eacute>> perchè i dati UTF-8 hanno un altro significato quando sono decodificati come
dati Latin-1.
...
Perl supporta i numeri sia come valori interi che floating-point. Potete rappresentarli
...
Anche se in Perl 5 potete scrivere esplicitamente valori floating-point
...
Considerate l'alternativa più leggibile.
...
=begin programlisting
    ...
    my $name = undef;   # assegnamento superfluo
    my $rank;           # contiene anche undef
...
--ad esempio interpolarlo in uno stringa--causa un warning di C<uninitialized
value>:

=begin programlisting

    my $undefined;
    my $defined = $undefined . '... e così via';
...
=begin programlisting
    ...
    my $status = 'ho il raffreddore';

    say B<defined> $status;  # 1, che rappresenta il valore vero
...
C<get_all_clown_hats()> in contesto lista. Questa chiamata restituisce una lista.

L'assegnamento alla lista vuota fa perdere tutti i valori della lista, ma
tale assegnmento ha luogo in contesto scalare, che valuta il numero di
elementi nella parte destra dell'assegnamento. Di conseguenza, C<$count> contiene
il numero di elementi nella lista restituita da C<get_all_clown_hats()>.

Se per il momento trovate questo concetto un po' confuso, non preoccupatevi. Man mano che capirete come
le funzionalità di progetto fondamentali del Perl si incastrano tra loro nella practica, tutto diventerà
più chiaro.
...
Usate l'operatore di range per creare liste di letterali in forma compatta:
...
(C<#>), perchè non solo tali caratteri sono rari in un C<qw()>, ma la loro presenza
...
=begin programlisting
    ...
    # say impone contesto lista
    say context();

    # il contesto void è triviale
    context();
