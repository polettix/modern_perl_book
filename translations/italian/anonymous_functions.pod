...
un nome oppure è anonima usando l'introspezione:
...
La funzione C<sub_name> del modulo CPAN C<Sub::Identify> semplifica questa
introspezione.
...
funzioni. Considerate il modulo CPAN C<Test::Fatal>:
...
La funzione C<exception()> prende una funzione anonima come primo argomento.
Le chiamate qui sopra sono equivalenti a:
...
... ma sono più agevoli da leggere.

Questa sintassi vi permette anche di passare riferimenti a funzioni con nome:
...
... perchè il prototipo modifica il modo in cui il parser di Perl 5 interpreta questo
