...
estrarli) usate la freccia di dereferenziazione:
...
Analogamente ai riferimenti ad array, potete sostituire la freccia di dereferenziazione con un altro
sigillo scalare, ad esempio C<$$rif_a_colori{$colore}>.
...
In Perl 5.12, questo codice funziona se prima avete scritto C<use IO::File;>. Prima del 5.12,
questi oggetti erano istanze di C<IO::Handle>, e quindi dovevate specificare tale classe nello C<use>.

Potreste anche vedere vecchio codice che usa riferimenti a typeglob, come il seguente:
...
=begin programlisting
    ...
    my $fh = do
    {
        local *FH;
        open FH, "> $file"
            or die "Non posso scrivere su '$file': $!\n";
        B<\*FH>;
    };

=end programlisting

Questo idioma è antecedente alla comparsa dei filehandle lessicali come parte di Perl 5.6.0 nel
...
=begin programlisting
    ...
    sub mostra_scope
    {
...

        {
            ...
            $fh->say( 'qui il file è aperto' );
        }

        say 'qui il file è chiuso';
    }
...
sorprese per il chiamante. Se usate strutture dati nidificate o altri riferimenti
