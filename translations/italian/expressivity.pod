...
Quando Larry Wall creò il Perl, i suoi studi di linguistica e lingue naturali
ne influenzarono la progettazione. In particolare, il Perl vi dà una enorme libertà
nel modo di risolvere i problemi, a seconda dello stile del vostro gruppo di lavoro, del tempo a vostra disposizione, del periodo di vita atteso
per il programma, e persino di quanto vi sentite creativi. Potete scrivere un pezzo di codice semplice e lineare
...
Laddove altri linguaggi impongono una soluzione "corretta" per la scrittura di ogni operazione, il Perl vi permette di ottimizzare secondo il criterio è più importante per voi.
...
C<$first_num> e C<$second_num>, Perl li converte in valori numerici (L<numeric_coercion>). Se avete espresso l'intenzione di trattarli come numeri
usando un operatore numerico, Perl sarà felice di accontentarvi.

X<DWIM>
X<principio di minima sorpresa>

Gli adepti del Perl spesso chiamano questo principo I<DWIM>, ovvero I<do what I mean>.  Un altro modo
...
Per esempio, un programmatore Perl esperto potrebbe triplicare una lista di numeri scrivendo:
...
L'esperienza con la programmazione in Perlvi aiuterà a focalizzarvi sul I<che cosa> volete ottenere piuttosto che sul I<come> ottenerlo.
