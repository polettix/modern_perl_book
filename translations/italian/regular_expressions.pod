...
=begin programlisting
    ...
    say 'Ho trovato una stella!' if $name =~ B</stella/>;
...
I<atomo>. Può combaciare o non combaciare.

X<operatori; C<=~>>
X<C<=~>; bind a regex>
X<operatori; C<!~>>
X<C<!~>; bind a regex negato>
...
comprare il latte su un elicottero da combattimento--ma Perl vi permette di decidere che cosa vi sembra
...
La funzione C<like> di C<Test::More> testa se c'è un match tra il primo argomento e la
...
X<C<?>; quantificatore zero or one di regex>
...
Ogni atomo in una espressione regolare seguito dal carattere C<?> significa "fai il match
con questo atomo zero o una volta". L'espressione regolare qui sopra trova un match se zero o una occorrenza del
carattere C<g> precedono immediatamente un carattere C<a>, quindi il match ha successo con le sottostringhe letterali C<miao> e C<mio>.
...
In teoria non c'è alcun limite al massimo numero di occorrenze dell'atomo quantificato per cui
...
Per quanto ciò possa sembrare curioso, vi permette di specificare componenti opzionali di una
...
di C<*>. La precisione di intenti spesso migliora la chiarezza.
...
ma la maggior parte dei programmi usano i primi molto più spesso dei secondi.
...
più caratteri possibili della stringa in input. Questo punto è particolarmente pericoloso. Considerate un
...
Il modificatore di quantificatori C<?> rende parsimoniosi i quantificatori greedy:
...
il più I<corto> dei match poteziali e aggiunge altri caratteri a quelli
...
Unate C<+?> per fare il match di una o più occorrenze in modo non-greedy:
...
ma non può sostituire il ruolo di essere esposti a una imponente mole di test con dati reali.
...
l'avvertmento dato sopra; molti principianti lo dimenticano. Una semplice ricerca con regex--che ignora
...
i caratteri alfanumerici (L<unicode>) e l'underscore:
...
Usate il metacarattere C<\s> per riconoscere la spaziatura, che si tratti di un semplice carattere di spazio, di un tab,
...
=begin programlisting
    ...
    my $non_unªvocale_ascii = qr/[^aeiou]/;

=end programlisting

=begin tip Metacaratteri nelle Classi di Caratteri

Usate un accento circonflesso in qualunque posizione eccetto che la prima per includerlo in una
...
unità più grandi e servono anche a catturare delle porzioni della stringa del match. Per riconoscere le parentesi
...
match durante l'applicazione di un'espressione regolare e accedervi in seguito, per esempio
un numero telefonico in una stringa contenente informazioni su un vostro contatto:
...
Le regex tendono a sembrare un'accozzaglia di segni di punteggiatura se non ne raggruppate diverse parti
in singoli pezzi. La sintassi delle catture con nome è la seguente:
...
resto della cattura è un'espressione regolare. Quando il match di un pattern contenente la cattura
ha successo, Perl memorizza la parte della stringa che corrisponde
al pattern interno alla cattura nella variabile predefinita C<%+>. In tale hash, la chiave è
il nome della cattura e il valore è l'appropriata porzione di stringa.
...
Invece, le sottostringhe catturate vengono memorizzate da Perl in una serie di variabili predefinite.
La parte di match della I<prima> cattura trovata finisce in C<$1>, la seconda in
C<$2> e così via. Il conteggio di una cattura avviene quando si incontra la sua parentesi I<aperta>;
...
Le catture con nome aumentano il grado di manutenibilità delle regex.

=begin tip Composizione Sicura di Catture

Le catture con nome I<possono> dar luogo a collisioni di nome, sebbene queste siano di gran lunga meno frequenti
delle collisioni di numero. Per evitarle, usate le catture con nome solo nelle regex più esterne.

=end tip

In contesto lista, il match di una regex con una stringa restituisce una lista di tutte le sottostringhe
catturate:
...
=begin programlisting
    ...
    my $maiale_e_fagioli = qr/\maiale?.*fagioli/;
...
Alcune regex devono riconoscere una cosa oppure un'altra. Il metacarattere di I<alternativa> (C<|>)
cattura questa situazione:
...
Il metacarattere di alternativa indica che il match deve essere fatto con l'uno o l'altro dei frammenti che lo precedono e lo seguono.
...
carattere C<e> oppure C<b>, seguito da C<agioli>, le alternative includono sempre l'I<intero>
...
Una espressione regolare convertita in stringa è racchiusa in un raggruppamento non-catturante;
...
carattere letterale punto anzichè al'atomo "fai il match con qualunque cosa eccetto un carattere
di a capo esplicito".
...
X<C<\Q>; metacarettere di disattivazione dei metacaratteri nelle regex>
X<C<\E>; metacarattere di riattivazione dei metacaratteri nelle regex>
X<regex; metdisattivazione dei metacaratteri>

I I<caratteri di disattivazione dei metacaratteri> (C<\Q> and C<\E>) disattivano
l'interpretazione dei metacaratteri nella parte di regex che si trova tra di essi. Possono essere particolarmente utili
quando ricevete l'espressione regolare da una sorgente di cui non avete il controllo quando scrivete
il programma:
...
anzichè applicare dei quantificatori greedy.
...
e niente di più, potete usare un'asserzione di limite di parola:
...
X<C<(?!...)>; asserzione di lunghezza zero di negazione in avanti nelle regex>
...
Il costrutto C<< (?<!...) >> contiene un pattern di lunghezza fissa. Potete anche
...
Molti modificatori possono essere utilizzati per cambiare il comportamento degli operatori delle espressioni regolari.
Tali modificatori compaiono alla fine degli operatori di match, di sostituzione a C<qr//>.
Per esempio, per attivare l'insensibilità tra maiuscole e minuscole dn un match:
...
Il primo C<like()> fallisce, perchè le due stringhe contengono lettere diverse.
Invece il secondo C<like()> ha successo, perchè il modificatore C</i> fa sì che la regex
...
(perchè il pattern non viene riconosciuto), il risultato è una copia identica
all'originale:
...
=begin programlisting
    ...
    my $statocopia = $stato
                   =~ s/fegato e cipolle/bratwurst/r;
    ...
    unlike( $statocopia,  qr/bratwurst/, 'niente wurstel'  );

=end programlisting

X<C</x>; modificatore di regex di incremento leggibilità>
X<regex; modificatore C</x>>
X<regex; incremento leggibilità>

Il modificatore C</x> vi permette di inserire spaziatura e commenti aggiuntivi nei
patterns. Quando questo modificatore è attivo, il motore di regex ignora
...
=begin programlisting
    ...
    # un po' di pace nella proprieta` della Mitchell
    my $contenuto = slurp( $file );
    $contenuto    =~ s/Scarlett O'Hara/Camomilla Calmante/g;
...
i numeri telefonici americani contenuti in un file mal strutturato, potreste scrivere:
...
globale visto prima potrebbe essere reso più robusto nel suo tentativo di rimpiazzare tutte le occorrenze del nome
della sventurata protagonista:

=begin programlisting

    # un po' di pace nella proprieta` della Mitchell
    my $contenuto = slurp( $file );
    $contenuto    =~ s/Scarlett( O'Hara)?}
...
del risultato dell'espressione, ma solo i golfer Perl tendono a usare C</ee> o
qualunque altra cosa ancora più complessa.
