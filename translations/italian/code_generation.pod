...
I programmatori principianti scrivono più codice del necessario, in parte perchè
non hanno familiarità col linguaggio, le librerie e gli idiomi, e in parte perchè
...
Man mano che miliorate come programmatori, vi troverete a scrivere sempre meno codice per risolvere uno stesso
problema. Userete delle astrazioni migliori e scriverete codice più generico. Potrete
riusare il codice--e quando arriverete ad aggiungere funzionalità cancellando del codice potrete considervarvi
davvero soddisfatti.

X<metaprogrammazione>
X<generazione di codice>

La scrittura di programmi che scrivono programmi al vostro posto--datta I<metaprogrammazione> o I<generazione
...
La tecnica più semplice di generazione di codice consiste nel definire una stringa contenete un
...
Usa anche meno menoria dato che tutte le istanze della
...
L'assegnamento in una tabella dei simboli tramite stringa, anzichè tramite un nome letterale di variabile,
...
Se il nome del metodo è un letterale stringa nel vostro codice sorgente, anzichè il valore
...
... dato che il blocco C<BEGIN>  viene eseguito I<prima> dell'assegnmento del
valore stringa a C<$package_desiderato>.  Il risulato è un'eccezione
