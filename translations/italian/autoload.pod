...
non definita. Ora aggiungete una funzione con nome C<AUTOLOAD()>:
...
Se rieseguite ora il programma, non succede nulla.  Perl chiama la funzione
C<AUTOLOAD()> di un package--se esiste-- ogni volta che il normale meccanismo di chiamata
non trova una funzione. Modificate C<AUTOLOAD()> in modo da stampare un messaggio:
...
alla funzione non definita. Potete manipolarli a vostro piacimento:
...
Il I<nome> della funzione non definita è memorizzato nella variabile pseudo-globale
C<$AUTOLOAD>:
...
Con la dichiarazione C<our> (L<our>), lo scope della variabile è il corpo di
C<AUTOLOAD()>. La variabile contiene il nome fully-qualified della funzione
...
con il I<nome> del metodo non definito. Installando tale chiusura
...
Sebbene questo approccio sia più pulito e quasi sempre più trasparente di
...
naE<iuml>ve alla generazione di metodi a tempo di esecuzione significa tra l'altro che il metodo C<can()>
...
Le dihiarazioni in avanti sono utili solo nei due casi piuttosto rari dell'uso degli attributi e dell'autoload (L<autoload>).
